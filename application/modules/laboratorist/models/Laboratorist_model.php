...
class Laboratorist_model extends CI_model
{
    ...
    function getLaboratoristByIonUserId($ion_user_id)
    {
        $this->db->where('hospital_id', $this->session->userdata('hospital_id'));
    ...
    public function updateAvailability($laboratorist_id, $available)
    {
        $this->db->set('available', $available);
        $this->db->where('id', $laboratorist_id);
        return $this->db->update('laboratorist');
    }
}
