...
class Ion_auth_model extends CI_Model
{
    ...
    public function login($identity, $password, $remember = FALSE)
    {
        ...
        $query = $this->db->select($this->identity_column . ', username, email, id, password, active, last_login, hospital_ion_id')
            ->where($this->identity_column, $identity)
            ->limit(1)
            ->order_by('id', 'desc')
            ->get($this->tables['users']);
    ...
    public function set_session($user)
    {
        ...
        if (!empty($user->hospital_ion_id)) {
            $hospital = $this->db->get_where('hospital', array('ion_user_id' => $user->hospital_ion_id))->row();
            if (!empty($hospital)) {
                $session_data['hospital_id'] = $hospital->id;
            } else {
                // If not found in hospital table, fallback to the ID itself (legacy behavior?)
                // But usually this means data issue. We'll leave it empty to trigger Doctor check.
            }
        }

        if (empty($session_data['hospital_id'])) {
            // Check if doctor
            $doctor = $this->db->get_where('doctor', array('ion_user_id' => $user->id))->row();
            if (!empty($doctor)) {
                $session_data['hospital_id'] = $doctor->hospital_id;
            }
        }

        if (empty($session_data['hospital_id'])) {
            $groups_ids = $this->db->get_where('users_groups', array('user_id' => $user->id))->row();
            if ($groups_ids->group_id == '1') { // Super Admin
                $session_data['hospital_id'] = 'superadmin';
            } elseif ($groups_ids->group_id == '11') { // Admin
                // If admin wasn't resolved via hospital_ion_id, try their own ID as hospital_ion_id?
                $hospital = $this->db->get_where('hospital', array('ion_user_id' => $user->id))->row();
                if (!empty($hospital)) {
                    $session_data['hospital_id'] = $hospital->id;
                } else {
                    $session_data['hospital_id'] = $user->id;
                }
            } else {
                $session_data['hospital_id'] = '';
            }
        }
