      ...
      success: function (response) {
        ...
        try {
          ...
          if (response.patient.age != null) {
            var ageParts = response.patient.age.split("-");
            patientAge = ageParts[0] || "0";
          }
          
          var option = new Option(
            response.patient.name + " (ID: P" + (response.patient.hospital_patient_id || response.patient.id) + " - Phone: " + (response.patient.phone || "") + " - Age: " + patientAge + ")",
            response.patient.id,
            true,
            true
          );
