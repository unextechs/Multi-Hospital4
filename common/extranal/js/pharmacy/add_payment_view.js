...
$(document).ready(function () {
    ...
    $("#my_multi_select4").select2({
        ...
        allowClear: true,
        ajax: {
            url: base_url + 'medicine/getMedicineForPharmacyMedicine',
            type: "post",
            dataType: 'json',
            delay: 250,
            data: function (params) {
                console.log('Regular view AJAX request with search term:', params.term);
                return {
                    searchTerm: params.term // search term
                };
            },
            processResults: function (response) {
                console.log('Regular view AJAX response:', response);
                ...
                return {
                    results: response.map(item => {
                        console.log('Processing regular item:', item);
                        
                        // Check if item has required properties
                        if (!item || !item.id || !item.text) {
                            console.error('Invalid regular item:', item);
                            return null;
                        }
            ...
            error: function(xhr, status, error) {
                console.error('Regular view AJAX error:', error);
